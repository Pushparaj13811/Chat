...
const ForwardMessageModal = ({ isOpen, onClose, message }) => {
  ...
  const filteredUsers = availableUsers.filter(user => {
    ...
    // Search by fullName or email with null checks
    const query = searchQuery.toLowerCase();
    const nameMatch = user.fullName ? user.fullName.toLowerCase().includes(query) : false;
            ...
            <div className="flex items-center mb-2">
              <div className="avatar mr-2">
                <div className="w-6 h-6 rounded-full">
                  <img 
                    src={message.senderId === authUser?._id ? (authUser?.profilePic || "/avatar.png") : (message.senderProfilePic || "/avatar.png")} 
                    alt="Sender" 
                    onError={(e) => {
